...
function makeHomeView() {
    fetch("https://the-music-vault.herokuapp.com/albums")
    .then(res => res.json())
    .then(albums => {
...
function makeHomeViewFromJson(albums) {
    ...
    flipCardEl.forEach(album => {
        ...
        deleteBtn.addEventListener("click", () => {
            ...
            fetch("https://the-music-vault.herokuapp.com/albums" + albumIdEl.value, {
                method: 'DELETE'
            })
            .then(res => res.json())
            .then(newAlbums => {
    ...
    addAlbumBtn.addEventListener("click", () => {
        if (albumTitleInput.value != "" && imageUrlInput.value != "" && recordLabelInput.value != "") {
            ...
            fetch(`https://the-music-vault.herokuapp.com/albums/addAlbum`, {
                method: 'POST',
                headers: {
...
function makeAlbumView(album) {
    ...
    addNewSongBtn.addEventListener("click", () => {
        ...
        if (songTitleInput.value != "" && songUrlInput.value != "" && durationInput.value != "" && artistNameInput.value != ""){
            ...
            fetch(`https://the-music-vault.herokuapp.com/albums/${album.id}/addSong`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    albumCmtBtn.addEventListener("click", () => {
        if(authorInput.value!="" && ratingInput.value!="" && commentInput.value!=""){
            ...
            fetch(`https://the-music-vault.herokuapp.com/albums/${album.id}/addComment`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    cardEl.forEach(song => {
        ...
        editSongBtn.addEventListener("click", () => {
            ...
            if(updateSongInput.value!=""){
                ...
                fetch("https://the-music-vault.herokuapp.com/songs/" + songIdEl.value, {
                    method: "PATCH",
...
function makeSongView(songJson, albumJson) {
    ...
    songCmtBtn.addEventListener("click", () => {
       if(songAuthorInput.value!=""&& songRatingInput.value!="" && songCommentInput.value!="" ){
            ...
            fetch(`https://the-music-vault.herokuapp.com/songs/${songJson.id}/addComment`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
